...
<SCRIPT type="text/javascript">
	...
	var src = alasql('select * into ? from xlsx("assets/Data.xlsx",{range:"X23:Y176"})', [ indexesData, ],
			function(data_xlsx) {
			...
			.ready(
					...
					function() {
						...
						var data = [
								...
								[ "62", "Category", "Energy Use (kWh/y)", "Energy Cost (£/y) ",
										"CO2 emissions (kg/y)", , , , ],
								[ "63", "Servers", '=CEILING(SUM(I4:I6),1)', "=CEILING((C63*C72),1)",
										"=CEILING((C63*C73),1)" ],
								[ "64", "PCs", '=CEILING(SUM(I11:I19),1)', "=CEILING((C64*C72),1)",
										"=CEILING((C64*C73),1)" ],
								[ "65", "Networks", "=CEILING(SUM(I24:I30),1)", "=CEILING((C65*C72),1)",
										"=CEILING((C65*C73),1)" ],
								[ "66", "Telephony", "=CEILING(SUM(I35:I37),1)", "=CEILING((C66*C72),1)",
										"=CEILING((C66*C73),1)" ],
								[ "67", "Imaging", "=CEILING(SUM(I42:I51),1)", "=CEILING((C67*C72),1)",
										"=CEILING((C67*C73),1)" ],
								[ "68", "AV", "=CEILING(SUM(I56:I58),1)", "=CEILING((C68*C72),1)",
										"=CEILING((C68*C73),1)" ],
								[ "69", "TOTAL", "=CEILING(SUM(C63:C68),1)", "=CEILING((C69*C72),1)",
										"=CEILING((C69*C73),1)" ], [ "70", "", , , , , , , '' ],
								[ "71	", "", , , , , , , '' ],
						...
						var hot = new Handsontable(container,
								{
									...
									cells : function(row, col, prop) {
										...
										if (row === 1 && col === 2 || row === 2 && col === 2 || row === 10
												...
												&& col === 2 || row === 13 && col === 2 || row === 14
												&& col === 2 || row === 15 && col === 2 || row === 16
												&& col === 2 || row === 17 && col === 2 || row === 23
												&& col === 2 || row === 24 && col === 2 || row === 25
												&& col === 2 || row === 26 && col === 2 || row === 27
												&& col === 2 || row === 28 && col === 2 || row === 29
												&& col === 2

												|| row === 41 && col === 2 || row === 42 && col === 2
												|| row === 43 && col === 2 || row === 44 && col === 2
												|| row === 45 && col === 2 || row === 46 && col === 2
												|| row === 47 && col === 2 || row === 48 && col === 2
												|| row === 49 && col === 2 || row === 50 && col === 2
												|| row === 55 && col === 2 || row === 56 && col === 2
												|| row === 57 && col === 2 || row === 34 && col === 2
												|| row === 35 && col === 2 || row === 36 && col === 2
					...
					/* 	$.getJSON("http://ip-api.com/json/?fields=countryCode", function(data) {
						...
						/* $.getJSON("http://freegeoip.net/json/", function(data) { */
							...
							alasql('select * from xlsx("assets/Data.xlsx",{range:"V23:W176"})', [], function(
									data_xlsx) {
								//console.log(data_xlsx);
								//	alert(data_xlsx[59].W)

								resultObject = (search(country, data_xlsx)) / 1000;
								alert("Your country code is: " + country + ". Emissions factor is: "
										+ resultObject)
